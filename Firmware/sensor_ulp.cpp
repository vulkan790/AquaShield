...
{
    // aqsh_ulp.s
    const ulp_insn_t adc_program[] = {
        I(READ_ADC, 7),                //читаем по каналу на пине 35
        I(STORE, 0, &ulp_wtr),
        I(LD, 1, &c_wtr_trshld),
        I(SUB, 2, 0, 1),
        I(JUMP, 3, 0, GE),             // если >=0 → wakeup
        I(HALT),
        I(WAKE),
        I(HALT)
    };
    
    ULPRun(adc_program, sizeof(adc_program)/sizeof(ulp_insn_t), &ulp_wtr, true);
    
    // 500 ms
    ulp_set_wakeup_period(0, 500000);
    
#ifdef DEBUG_SENSOR
    Serial.println("[-setup_ulp] ULP started");
#endif
}
